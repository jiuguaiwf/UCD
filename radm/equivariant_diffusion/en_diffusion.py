...
def adaptive_langevin_variance(u_t, u_max, gamma=10.0, sigma_min=0.7, sigma_max=1.0):
    ...
    sigma_squared = sigma_min + (sigma_max - sigma_min) * s
    return sigma_squared.to(u_t.device)*0.8
...
class EnLatentDiffusion(EnVariationalDiffusion):
    ...
    @torch.no_grad()
    def uncertainty_sample(self, n_samples, n_nodes, node_mask, edge_mask, 
                           context, fix_noise=False, variance_cal_times = 20, 
                           dynamic_weights = False, u_max = 1):
        """
        Draw samples from the generative model.
        """
        z_x, z_h, traj_uncertainty = super().uncertainty_sample(n_samples, n_nodes, node_mask, 
                                              edge_mask, context, fix_noise,
        ...
        return pred_x, {'categorical': pred_cat, 'integer': pred_int}, traj_uncertainty
