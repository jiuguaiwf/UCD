...
def resolve_geom_data_file(preferred_path=None):
    candidates = []
    if preferred_path:
        candidates.append(preferred_path)

    env_path = os.environ.get('GEOM_DRUGS_PATH')
    if env_path:
        candidates.append(env_path)

    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    candidates.append(os.path.join(repo_root, 'data', 'geom', 'geom_drugs_30.npy'))
    ...
    raise RuntimeError(
        'Could not find GEOM-Drugs data file "geom_drugs_30.npy". '
        'Place it under data/geom/, pass a config with data_file set, '
        'or set the GEOM_DRUGS_PATH environment variable.'
    )
...
def retrieve_dataloaders(cfg):
    if 'qm9' in cfg.dataset:
        ...
        data_file = resolve_geom_data_file(getattr(cfg, 'data_file', None))
