...
def uncertainty_sample(args, device, generative_model, dataset_info,
           prop_dist=None, nodesxsample=torch.tensor([10]), context=None,
           fix_noise=False, variance_cal_times = 10, 
           dynamic_weights = True, u_max = 1):
    ...
    if args.probabilistic_model == 'diffusion':
        x, h, traj_uncertainty = generative_model.uncertainty_sample(batch_size, max_n_nodes, 
    ...
    return one_hot, charges, x, node_mask, traj_uncertainty
...
def sample_sweep_conditional(args, device, generative_model, dataset_info, prop_dist, n_nodes=19, n_frames=100):
    ...
    one_hot, charges, x, node_mask = uncertainty_sample(args, device, generative_model, dataset_info, prop_dist, nodesxsample=nodesxsample, context=context, fix_noise=True)
    return one_hot, charges, x, node_mask