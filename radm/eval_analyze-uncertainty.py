...
def analyze_and_save(args, eval_args, device, generative_model,
                     ...
                     batch_size=10, save_to_xyz=False, variance_cal_times = 20,
    ...
    for i in range(int(n_samples/batch_size)):
        ...
        one_hot, charges, x, node_mask, traj_uncertainty = uncertainty_sample(
            ...
            nodesxsample=nodesxsample, variance_cal_times = variance_cal_times,
            dynamic_weights = dynamic_weights, u_max = u_max)
        
        uncertainty_test += sum(traj_uncertainty) / len(traj_uncertainty) / (n_samples/batch_size)
...
def main():
    ...
    parser.add_argument('--variance_cal_times', type=int, default=20,
                        help='Number of MC_dropout')
    parser.add_argument('--dynamic_weights', type=int, default=1,
                        help='Whether enable uncertainty weights in Lang. Dynamics')
    parser.add_argument('--u_max', type=float, default=0.00037,
                        help='Threshold for uncertainty')
    
    parser.add_argument('--iterations', type=int, default=3,
                        help='iterations for sampling test')
    ...
    for iter in range(eval_args.iterations):
        # Analyze stability, validity, uniqueness and novelty
        stability_dict, rdkit_metrics, uncertainty_dict = analyze_and_save(
        ...
        if rdkit_metrics is not None:
            ...
            with open(join(eval_args.model_path, f'Uncertainty_{bool(eval_args.dynamic_weights)}_u_max={eval_args.u_max}_N={eval_args.variance_cal_times}_log.txt'), 'a') as f:
                print("\nValidity %.4f, Uniqueness: %.4f, Novelty: %.4f" % (rdkit_metrics[0], rdkit_metrics[1], rdkit_metrics[2]), file=f)
                print(stability_dict, file=f)
                print(uncertainty_dict, file=f)
