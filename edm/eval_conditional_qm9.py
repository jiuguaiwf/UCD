...
def get_generator(dir_path, dataloaders, device, args_gen, property_norms):
    ...
    # Older checkpoints may not store the EGNN variant in args.pickle.
    # Infer it from the saved parameter names so we rebuild the matching model.
    if not hasattr(args_gen, 'egnn_variant'):
        has_dropout_head = any(k.startswith('dynamics.egnn.final_layer.') for k in model_state_dict)
        has_base_head = any(k.startswith('dynamics.egnn.embedding_out.') for k in model_state_dict)
        if has_dropout_head and not has_base_head:
            args_gen.egnn_variant = 'dropout'
        else:
            args_gen.egnn_variant = 'base'
